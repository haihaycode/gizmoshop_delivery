    ...
    <div v-for="order in orders" :key="order.id"
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-gray-600">
          ...
          {{ order.account ? order.account.fullname : "Không có thông tin" }}
        </p>
      ...
      <div class="flex items-center gap-4">
        <button @click="$emit('view-details', order)" class="text-blue-600 hover:underline text-xs sm:text-sm">
          Xem chi tiết
        </button>
        <input type="checkbox" :checked="selectedOrders.includes(order.id)"
          @change="handleCheckboxChange($event, order.id)" class="h-5 w-5 text-blue-500 border-gray-300 rounded" />
      </div>
    </div>
  </div>
</template>
    ...
    handleCheckboxChange(event, orderId) {
      ...
      if (isChecked) {
        ...
        this.$emit(
          ...
          this.selectedOrders.filter((id) => id !== orderId)
        );
      }
    },
  },
