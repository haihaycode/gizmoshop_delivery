<template>
  <transition name="fade">
    <div v-if="order" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 \">
      <div class="bg-white w-11/12 max-w-lg  max-h-[600px] rounded-lg shadow-lg p-6 relative overflow-y-auto">
        <button class="absolute top-2 right-2 text-gray-500 hover:text-gray-700" @click="$emit('closeDetail')">
          ✕
        </button>
        <h3 class="text-xl font-bold mb-4">
          Chi tiết đơn hàng: #MDH_{{ order.orderCode }}
        </h3>
        <p class="mb-2">
        ...
        <div class="flex space-x-2">
          <button class="w-1/2 bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition"
            @click="$emit('confirmDelivery', order)">
            Giao hàng
          </button>
          <button class="w-1/2 bg-red-500 text-white py-2 rounded-md hover:bg-red-600 transition"
